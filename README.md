...
**Micro-optimizations pack** *(`--micro-opts`)* — 21 small generic patches from WildKernels (20 verbatim, `optimise_memcmp` adapted to the 5.15 `WEAK_PI` entry scheme): arm64 mem/string ops, dcache pressure, alarmtimer wakeup time, s2idle attempts, forced `TCP_NODELAY`, socket buffers, f2fs/ext4 tuning, global wakelock timeouts, IRQ/cpuhotplug logspam silence, int_sqrt, DynamIQ buddy, PCI PME wakeups, scheduler scan order, f2fs GC sleep. Each applied only if it fits the branch (check `PATCH_STATUS.json` → `micro_opts` for the per-patch score). No Kconfig, no runtime toggle — the code changes are unconditional by design.
```bash
su -c "uname -r"
```
No `/proc` flag to check — verify via the build's patch-status summary instead. Note: `force_tcp_nodelay` trades slightly higher packet counts for lower latency on every TCP flow.

**OPlus mm module family** *(`--oplus-mm`)* — six loadable modules from OnePlus sm8550 (`async_reclaim_opt`: `kshrink_lruvecd`, `kshrink_slabd`, `pcppages_opt`, `abort_mm_opt`; `memload_opt`: `look_around`, `mapped_protect`): async reclaim daemons, drain/abort bypasses, look-around migration, mapped-file protection. All opt-in at `insmod` — the Image behaves like stock until loaded.
```bash
for m in kshrink_lruvecd kshrink_slabd pcppages_opt abort_mm_opt look_around mapped_protect; do su -c "insmod /path/to/oplus_bsp_$m.ko"; done
su -c "lsmod | grep oplus_bsp"
```
