...
/*
 * Vendored from OnePlusOSS/android_kernel_modules_and_devicetree_oneplus_sm8850
 * branch oneplus/sm8850_b_16.0.0_oneplus_15, path
 * vendor/oplus/kernel/storage/storage_feature_in_module/common/oplus_uprobe/.
 * Verbatim (Kconfig verbatim; Makefile adapted - GCOV_PROFILE dropped, it
 * needs CONFIG_GCOV_KERNEL which is off here). Kprobe-resolved uprobe API,
 * no other OPlus dependency; needs CONFIG_UPROBES (enabled by our build).
 */
...
typedef struct uprobe * (*uprobe_register_t)(struct inode *, loff_t, loff_t, struct uprobe_consumer *);
typedef void (*uprobe_unregister_t)(struct uprobe *, struct uprobe_consumer *);
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0))
int __nocfi uprobe_register_fn(struct oplus_uprobe *ou, struct inode *inode, unsigned long offset, struct uprobe_consumer *consumer)
{
	struct uprobe *uprobe;
	uprobe = uprobe_register_funcptr(inode, offset, 0, consumer);
	if (IS_ERR(uprobe)) {
		ou->uprobe = NULL;
...
void __nocfi uprobe_unregister_fn(struct oplus_uprobe *ou, struct uprobe_consumer *consumer)
{
	if (ou != NULL || ou->uprobe != NULL)
		uprobe_unregister_funcptr(ou->uprobe, consumer);
}
#else
	...
	*arg++ = '\0';
	...
	list_for_each_entry(comp, &uprobe_event_list, head) {
		...
		if (comp_inode == d_real_inode(path.dentry) && (comp->offset == offset)) {
			list_del(&comp->head);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0))
			uprobe_unregister_fn(comp, &comp->consumer);
#else
...
static int oplus_uprobe_register(struct oplus_uprobe *ou)
{
	...
	ret = uprobe_register_fn(ou, ou->inode, ou->offset, &ou->consumer);
	if (ret)
		ou->inode = NULL;
	return ret;
}
...
static struct kprobe uprobe_unregister_kp = {
	.symbol_name = "uprobe_unregister_nosync"
};
#else
static struct kprobe uprobe_unregister_kp = {
...
static int __init oplus_uprobe_init(void) {
	...
	if (ret < 0) {
		...
		path_put_funcptr = (path_put_t)path_put_kp.addr;
