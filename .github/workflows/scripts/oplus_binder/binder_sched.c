 ...
 * vendor/oplus/kernel/ipc/binder_sched.c. Verbatim.
 *
 * Porting notes (this project builds vanilla kernel/common GKI, not the
 * OnePlus tree):
 * - All sched-assist-coupled code is already upstream-guarded with
 *   IS_ENABLED(CONFIG_OPLUS_FEATURE_SCHED_ASSIST), so with sched assist
 *   absent every handler except the two below compiles to a near-no-op.
 * - The two always-active paths need nothing from OnePlus: the 5.15
 *   saved_priority fix sits on the standard Google vendor hook
 *   android_vh_binder_restore_priority, and PRIO_SKIP on
 *   android_vh_binder_priority_skip. Both hook declarations AND their
 *   binder.c call sites ship in vanilla kernel/common (no in-tree delta
 *   required - the OnePlus tree's own drivers/android/binder.c carries
 *   zero oplus modifications).
 */
